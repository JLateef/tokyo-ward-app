export const tokyoWardsNightlifeData = [
  {
    id: 'shinjuku',
    name: 'Shinjuku',
    nameJa: '新宿区',
    barsIzakayas: 2800,
    nightclubs: 45,
    lateNightDining: 380,
    footTrafficScore: 9.8,
    featuredVenues: ['Kabukicho', 'Golden Gai', 'Ni-chome'],
    wikiTitle: '歌舞伎町',
    imageUrl: 'https://media.timeout.com/images/105202965/image.jpg',
    description:
      'Shinjuku is home to Kabukicho, the largest entertainment district in Asia, a relentless grid of neon signs, hostess bars, izakayas, karaoke towers, and clubs that run well past sunrise. Golden Gai is perhaps Tokyo\'s most atmospheric bar neighborhood: around 200 tiny bars packed into six narrow alleys, each seating no more than a handful of people, each with its own obsessively curated aesthetic. Ni-chome, the compact LGBTQ+ district tucked behind the main entertainment zone, contains one of the world\'s highest concentrations of queer bars per square meter, an institution that has made Shinjuku a pilgrimage for visitors from across the globe. The ward\'s diversity of nightlife formats is unmatched: a standing ramen bar one night, a cocktail lounge on the 40th floor the next. No other ward in Tokyo offers a denser or more varied night out.',
    lat: 35.6938,
    lng: 139.7035,
  },
  {
    id: 'shibuya',
    name: 'Shibuya',
    nameJa: '渋谷区',
    barsIzakayas: 980,
    nightclubs: 38,
    lateNightDining: 210,
    footTrafficScore: 8.7,
    featuredVenues: ['Dogenzaka', 'Udagawacho Club Row', 'Daikanyama'],
    wikiTitle: '渋谷',
    imageUrl: 'https://tripguide.tokyo/wp-content/uploads/2025/04/shibuya-crossing-crowded.jpg',
    description:
      'Shibuya earned its global reputation on the visual drama of the crossing, but its nightlife runs deep into the surrounding streets and hills. The club district centered on Udagawacho and Dogenzaka is one of Asia\'s most significant for electronic music and DJ culture, with venues like Womb and Contact drawing international acts of real caliber. Daikanyama has evolved into perhaps the most stylistically influential bar neighborhood in Tokyo, where craft cocktail bars and record shops coexist with natural wine caves and all-night coffee counters. Shibuya\'s foot traffic after midnight is among the highest of any ward, fed by the station hub that connects it to the rest of the city. For club-focused nightlife, Shibuya is Shinjuku\'s only serious rival.',
    lat: 35.6639,
    lng: 139.6982,
  },
  {
    id: 'minato',
    name: 'Minato',
    nameJa: '港区',
    barsIzakayas: 1100,
    nightclubs: 30,
    lateNightDining: 260,
    footTrafficScore: 8.2,
    featuredVenues: ['Roppongi', 'Nishi-Azabu', 'Azabu-Juban'],
    wikiTitle: '六本木',
    imageUrl: 'https://static.gltjp.com/glt/data/article/21000/20571/20260326_063230_01c31f34_w1920.webp',
    description:
      'Minato\'s nightlife is defined by Roppongi, a district synonymous internationally with Tokyo after dark, home to some of the city\'s most famous clubs, rooftop bars, and a dense ecosystem of international-facing venues. The adjacent neighborhoods of Nishi-Azabu and Azabu-Juban offer a more restrained alternative, where underground cocktail bars and intimate jazz lounges serve Minato\'s wealthy residential community well into the early hours. The ward also contains pockets of discreet late-night dining in Hiroo and Moto-Azabu, beloved by diplomats, executives, and those who prefer their evenings without the Kabukicho crush. Roppongi\'s reputation for excess is well-earned but incomplete, Minato rewards those who venture off the main crossing. The ward\'s nightlife runs more expensive and international than Shinjuku\'s, a clear reflection of its character.',
    lat: 35.6581,
    lng: 139.7514,
  },
  {
    id: 'chuo',
    name: 'Chuo',
    nameJa: '中央区',
    barsIzakayas: 680,
    nightclubs: 12,
    lateNightDining: 160,
    footTrafficScore: 7.2,
    featuredVenues: ['Ginza', 'Tsukiji', 'Nihonbashi'],
    wikiTitle: '銀座',
    imageUrl: 'https://cdn.cheapoguides.com/wp-content/uploads/sites/2/2020/08/ginza-seiko-building-taxis-AP-1024x600.jpg',
    description:
      'Chuo may not be Tokyo\'s first nightlife association, but Ginza, the ward\'s most famous district, harbors one of the city\'s most concentrated and consistently high-caliber after-dark cultures. Hundreds of intimate cocktail bars, sake specialists, and high-end establishments line the side streets off Chuo-dori, operating at a pitch that is simultaneously exclusive and deeply welcoming to those in the know. The Tsukiji and Ningyocho neighborhoods add a more accessible dimension, with izakayas and local bars that have served the ward\'s working heritage for generations. Late-night dining in Ginza is exceptional: sushi counters, tempura bars, and omakase restaurants that stay open past midnight cater to a clientele for whom the evening only begins after 10pm. Chuo\'s nightlife is quieter than Shinjuku\'s, but for cocktails and fine dining late into the night, it remains one of the city\'s most rewarding destinations.',
    lat: 35.6704,
    lng: 139.7730,
  },
  {
    id: 'taito',
    name: 'Taito',
    nameJa: '台東区',
    barsIzakayas: 740,
    nightclubs: 6,
    lateNightDining: 175,
    footTrafficScore: 7.0,
    featuredVenues: ['Hoppy Street', 'Asakusa Bar Alley', 'Ueno Ameyoko'],
    wikiTitle: '浅草',
    imageUrl: 'https://dynamic-media-cdn.tripadvisor.com/media/photo-o/31/ab/23/25/caption.jpg?w=1100&h=1100&s=1',
    description:
      'Taito\'s nightlife is rooted in the izakaya culture of old Tokyo, unpretentious, communal, and deeply atmospheric in a way that no other ward quite replicates. Hoppy Street in Asakusa is one of the most sociable drinking destinations in the city: a narrow lane where salarymen, tourists, and locals of all ages share outdoor benches over endless plates of offal stew and plastic cups of hoppy, a low-alcohol malt drink that has defined the street for decades. The Ueno area adds a grittier dimension, a more underground bar scene running behind the station through the Ameyoko market alleys that stays busy until the early hours. Taito lacks the clubs and rooftop bars of Shibuya and Roppongi, but for an evening of no-frills, cheap, loud drinking in surroundings that feel unchanged since the 1970s, it has no equal. This is Tokyo nightlife at its most genuine.',
    lat: 35.7126,
    lng: 139.7850,
  },
  {
    id: 'chiyoda',
    name: 'Chiyoda',
    nameJa: '千代田区',
    barsIzakayas: 480,
    nightclubs: 5,
    lateNightDining: 120,
    footTrafficScore: 6.2,
    featuredVenues: ['Marunouchi', 'Akihabara', 'Hibiya'],
    wikiTitle: '秋葉原',
    description:
      'Chiyoda\'s nightlife is largely a business-hours affair, Marunouchi and Hibiya after dark fill with salarymen at expense-account izakayas and whisky bars before the last train. Akihabara adds an unusual dimension: electronics floors close but maid cafes, game bars, and themed venues carry on into the evening, attracting a dedicated subculture of visitors. The ward\'s nightlife is real but geographically dispersed.',
    lat: 35.6938,
    lng: 139.7535,
  },
  {
    id: 'nakano',
    name: 'Nakano',
    nameJa: '中野区',
    barsIzakayas: 350,
    nightclubs: 2,
    lateNightDining: 80,
    footTrafficScore: 5.8,
    featuredVenues: ['Nakano Broadway area', 'Sun Mall arcade'],
    wikiTitle: '中野区',
    description:
      'Nakano\'s nightlife centers on the covered shopping arcade and its surrounding side streets, where affordable izakayas and dive bars serve the ward\'s young, creative, and cash-conscious community. It lacks the scale of Shinjuku, just minutes away by train, but has a loyal local drinking scene and an indie music venue presence that makes it a genuine neighborhood night out for those who know it.',
    lat: 35.7073,
    lng: 139.6659,
  },
  {
    id: 'sumida',
    name: 'Sumida',
    nameJa: '墨田区',
    barsIzakayas: 300,
    nightclubs: 3,
    lateNightDining: 72,
    footTrafficScore: 5.5,
    featuredVenues: ['Kinshicho', 'Ryogoku'],
    wikiTitle: '錦糸町',
    description:
      'Sumida has a handful of izakaya clusters around Ryogoku and Kinshicho, with Kinshicho in particular developing into a lively local bar district that draws a younger crowd seeking the energy of Shibuya at more approachable prices. Ryogoku\'s sumo culture generates a unique izakaya atmosphere on tournament days. Modest in scale but genuine in character.',
    lat: 35.7101,
    lng: 139.8017,
  },
  {
    id: 'setagaya',
    name: 'Setagaya',
    nameJa: '世田谷区',
    barsIzakayas: 320,
    nightclubs: 2,
    lateNightDining: 75,
    footTrafficScore: 5.2,
    featuredVenues: ['Sangenjaya', 'Shimokitazawa'],
    wikiTitle: '三軒茶屋',
    description:
      'Setagaya is a residential ward at heart, but its Sangenjaya and Shimokitazawa neighborhoods are two of Tokyo\'s most beloved bohemian districts, dense with live music venues, bars, vintage shops, and izakayas that attract a younger, creative crowd from across the city. For a ward better known for its parks, Setagaya\'s nightlife credentials are a genuine and pleasant surprise.',
    lat: 35.6462,
    lng: 139.6530,
  },
  {
    id: 'bunkyo',
    name: 'Bunkyo',
    nameJa: '文京区',
    barsIzakayas: 280,
    nightclubs: 2,
    lateNightDining: 70,
    footTrafficScore: 5.0,
    featuredVenues: ['Hongo', 'Nezu'],
    wikiTitle: '本郷_(文京区)',
    description:
      'Bunkyo\'s drinking scene is driven by the student and academic populations of the University of Tokyo, generating a cluster of affordable bars, izakayas, and live music venues in Hongo and Nezu. It is a ward for a civilised, early-finishing night out rather than a serious late-night push.',
    lat: 35.7079,
    lng: 139.7523,
  },
  {
    id: 'nerima',
    name: 'Nerima',
    nameJa: '練馬区',
    barsIzakayas: 220,
    nightclubs: 1,
    lateNightDining: 55,
    footTrafficScore: 4.5,
    featuredVenues: ['Nerima Station area', 'Hikarigaoka'],
    wikiTitle: '練馬区',
    description:
      'Nerima\'s nightlife is thin, primarily local izakayas and karaoke boxes serving commuters on the Seibu and Toei lines heading home after Shinjuku. The ward is best enjoyed during the day, in its parks.',
    lat: 35.7357,
    lng: 139.6523,
  },
  {
    id: 'edogawa',
    name: 'Edogawa',
    nameJa: '江戸川区',
    barsIzakayas: 150,
    nightclubs: 0,
    lateNightDining: 35,
    footTrafficScore: 3.5,
    featuredVenues: ['Nishi-Kasai', 'Koiwa'],
    wikiTitle: '江戸川区',
    description:
      'Edogawa is primarily a family and parks ward with minimal nightlife infrastructure. What exists is hyperlocal, neighborhood izakayas and karaoke boxes serving residents rather than visitors. Edogawa\'s strengths lie firmly elsewhere.',
    lat: 35.7069,
    lng: 139.8679,
  },
];
