...
const PARK_ICON = (
  <svg viewBox="0 0 32 32" fill="none" width="28" height="28">
    <ellipse cx="16" cy="12" rx="9" ry="8" fill="white" opacity="0.5" />
    <ellipse cx="10" cy="15" rx="6" ry="5.5" fill="white" opacity="0.5" />
    <ellipse cx="22" cy="15" rx="6" ry="5.5" fill="white" opacity="0.5" />
    <rect x="14.5" y="19" width="3" height="7" rx="1.5" fill="white" opacity="0.5" />
  </svg>
);
...
export default function WardDetail({ ward, rank, onBack }) {
    ...
    fetch(`https://ja.wikipedia.org/api/rest_v1/page/summary/${encodeURIComponent(ward.wikiTitle)}`)
      ...
      .then(data => {
        ...
        if (raw) {
          ...
          // Fall back to the English Wikipedia ward article which reliably has photos
  ...
  return (
    <div className="min-h-screen bg-gray-50 px-4 pt-10 pb-10 max-w-md mx-auto">
      ...
      {/* Hero row: image left — ward info right */}
        ...
        <div className="flex flex-col justify-center flex-1 min-w-0">
          <div className="flex items-center gap-1.5 mb-1">
            <div
              ...
              style={{ backgroundColor: rank === 1 ? '#085041' : '#1D9E75' }}
            >
          ...
          <div className="mt-1.5 flex flex-col gap-0.5">
            {[['museums', 'museums'], ['temples', 'temples & shrines'], ['nightlife', 'nightlife']].map(([key, label]) => (
      ...
      <div className="flex flex-wrap gap-1.5 mb-6">
        {ward.featuredParks.map(park => {
          const q = encodeURIComponent(park + ' Tokyo');
          return (
            <div key={park} className="flex items-center rounded-full text-xs font-medium overflow-hidden" style={{ backgroundColor: '#E1F5EE' }}>
              <span className="pl-2.5 pr-2 py-1" style={{ color: '#085041' }}>{park}</span>
              <div className="w-px self-stretch" style={{ backgroundColor: '#08504130' }} />
      ...
      >
        <div>
          <div className="text-2xl font-black leading-none" style={{ color: '#085041' }}>
            {ward.majorParks}
          </div>
          <div className="text-xs text-gray-500 mt-1">major parks</div>
        </div>
        ...
        <div>
          ...
          <div className="text-xs text-gray-500 mt-1">m² per resident</div>
        </div>
      </div>
        ...
        <div className="flex gap-2">
          {[
            ...
            <a
              ...
              style={{ borderColor: '#1D9E75', color: '#085041', backgroundColor: '#E1F5EE' }}
            >
              {label}
            </a>
          ))}
