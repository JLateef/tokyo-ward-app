...
const MUSEUM_ICON = (
  <svg viewBox="0 0 32 32" fill="none" width="26" height="26">
    <polygon points="16,4 28,11 4,11" fill="white" opacity="0.5" />
    <rect x="6" y="11" width="4" height="13" fill="white" opacity="0.5" />
    <rect x="14" y="11" width="4" height="13" fill="white" opacity="0.5" />
    <rect x="22" y="11" width="4" height="13" fill="white" opacity="0.5" />
    <rect x="4" y="24" width="24" height="3" rx="1" fill="white" opacity="0.5" />
  </svg>
);
    ...
    <div className="min-h-screen bg-gray-50 px-4 pt-10 pb-10 max-w-md mx-auto">
      ...
      <div className="flex gap-4 mb-5">
        ...
        <div className="flex flex-col justify-center flex-1 min-w-0">
          ...
          <div className="mt-1.5 flex flex-col gap-0.5">
            {[['parks', 'parks'], ['temples', 'temples & shrines'], ['nightlife', 'nightlife']].map(([key, label]) => (
      ...
      <div className="flex flex-wrap gap-1.5 mb-6">
        {ward.featuredMuseums.map(museum => {
          const q = encodeURIComponent(museum + ' Tokyo');
          return (
            <div key={museum} className="flex items-center rounded-full text-xs font-medium overflow-hidden" style={{ backgroundColor: LIGHT }}>
              <span className="pl-2.5 pr-2 py-1" style={{ color: DARK }}>{museum}</span>
      ...
      >
        <div>
          ...
          <div className="text-xs text-gray-500 mt-1">total museums</div>
        </div>
        <div className="w-px" style={{ backgroundColor: '#E1C4F7' }} />
        <div>
          <div className="text-2xl font-black leading-none" style={{ color: DARK }}>
            {ward.prominentMuseums}
          </div>
          <div className="text-xs text-gray-500 mt-1">nationally recognized</div>
        </div>
      </div>
